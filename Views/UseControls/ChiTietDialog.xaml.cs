            ...
            if (sender is System.Windows.Controls.Button btn && btn.DataContext is DriverProfile profile)
            {
                ...
                if (!string.IsNullOrEmpty(cleanText))
                {
                    try
                    {
                        ...
                        // 2. Khôi phục cửa sổ nếu đang minimize
                        ...
                        // 3. [QUAN TRỌNG] Force focus vào LDPlayer
                        // Trick: Focus Shell trước
                        ...
                        // 5. [MỚI] Verify focus thành công
                        IntPtr currentFocus = GetForegroundWindow();
                        if (currentFocus != _cachedEmulatorHandle)
                        {
                            Debug.WriteLine("⚠️ Focus failed, retrying...");
                            // Thử lại 1 lần nữa
                            SetForegroundWindow(_cachedEmulatorHandle);
                            await Task.Delay(200);
                        }
